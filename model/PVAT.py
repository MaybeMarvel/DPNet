...
class TB(nn.Module):
    def __init__(self):
        ...
        backbone = PVT.PyramidVisionTransformerV2(
            patch_size=4,
        ...
        checkpoint = torch.load("D:\Test_data\VIT\projectcode\model\pvt_v2_b3.pth")
...
class FCBFormer(nn.Module):
    def __init__(self, size=352,classes=32):
        ...
        self.PH = nn.Sequential(
            RB(64+32 , 64), RB(64, 64), nn.Conv2d(64, classes, kernel_size=1)
        )
    ...
    def forward(self, x):
        TB,_ = self.TB(x)
        FCB = self.FCB(x)
        up_TB = self.up_tosize(TB)
        x = torch.cat((up_TB, FCB), dim=1)
        out = self.PH(x)
        feature_Dict = {'TB': TB, 'up_TB': up_TB,
                        'FCB': FCB, 'X': x,
                        }
